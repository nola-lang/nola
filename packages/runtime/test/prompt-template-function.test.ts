...
import { ExtractContext, Frame, nolaRuntime, inferTypes as t } from "@nola-lang/runtime";
import { afterEach, describe, expect, it } from "vitest";
import { classicText } from "./helpers/model.js";

afterEach(() => nolaRuntime.reset());
const runtime = () => nolaRuntime.current();
const extract = () => new ExtractContext({ instruction: "p", type: { type: "string" }, loc: "1:1" }, runtime());
const TASK = [
  "TASK",
...
describe("infer-function prompt template", () => {
  it("replaces the CONTEXT block and appends the remainder when .next is not read", () => {
    ...
    const text = classicText(Frame.open(fn), extract());
    expect(text).toBe(
  ...
  it("a caller template wraps the callee's default block", () => {
    ...
    const text = classicText(callee, extract());
    expect(text).toBe(`[CONTEXT — inside callee(), x.tsi, called from the context above\n\n${TASK}]`);
  });
  ...
  it("an empty or throwing template is a definitive NOLA3014", () => {
    ...
    let caught: unknown;
    try {
      classicText(Frame.open(empty), extract());
    } catch (e) {
      caught = e;
    }
    ...
    expect(() => classicText(Frame.open(throwing), extract())).toThrow(/NOLA3014.*boom/);
  });
