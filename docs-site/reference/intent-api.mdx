...
```ts
// not-checked — the shapes .withProvider()/.withParams() accept (ProviderRef is exported from @nola-lang/runtime; ProviderParams is not re-exported — use it structurally)
