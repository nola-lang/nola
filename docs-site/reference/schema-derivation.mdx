...
`Choice<{ … }>`, `Scale<[ … ]>` and `Prob<{ … }>` ([Decision types](/docs/language/decision-types/)) derive to their own carrier nodes. The walk reads the criteria off the resolved type, so they survive `Partial`, `Pick`, `Omit` and generic instantiation. The schema is the answer object's structural shape plus an `x-nola-decision` keyword carrying the question — `{ kind: "choice", criteria }`, `{ kind: "scale", levels }` or `{ kind: "prob", criteria? }` — which decision providers read and every dialect passes through. Validation checks the answer: the label must be one sent, the distribution must sum to 1 within `1e-6`, the score must lie within `0 … levels.length - 1`, a probability within `0 … 1`; the runtime fills `levels` into a scale answer. Malformed criteria are [`NOLA2015`](/docs/reference/error-codes/#nola2015) at every kind of site; a JSDoc constraint tag on a `Prob` is [`NOLA2012`](/docs/reference/error-codes/#nola2012).
