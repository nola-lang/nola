...
From the `typescript-interop` layout, `npx nola-lang build --out dist` writes:
...
- The output **mirrors the source tree** — `src/person.tsi` becomes `dist/src/person.tsi.js`. Views of cross-file types are emitted beside it as `<name>.tsi.js` + `<name>.tsi.d.ts` (they re-export `./<name>.js`, so compile your plain `.ts` into the same tree).
...
The recipe, verified against a fresh `typescript-interop` project:
