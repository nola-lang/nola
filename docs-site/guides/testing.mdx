...
Nola code is deterministic to test when the provider is: a `mockProvider` or a replay ledger answers every ask, `forceModel` guarantees nothing escapes to the network, and your test runner imports the `.tsi` file like any other module. Every recipe on this page was run against a fresh starter project.
