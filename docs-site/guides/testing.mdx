...
description: Test Nola code deterministically — mockProvider, forceModel for hermetic CI, node:test or vitest, and record/replay ledgers.
sidebar:
  order: 60
---

Nola code is deterministic to test when the provider is: a `mockProvider` or a replay ledger answers every ask, `forceModel` guarantees nothing escapes to the network, and your test runner imports the `.tsi` file like any other module. Every recipe on this page was run against a fresh `typescript-interop` project.
...
export default defineConfig({
  ...
  // hermetic: with NOLA_TEST=1 EVERY ask goes to `mock`, even ones pinned with ask with / .withModel()
  forceModel: process.env.NOLA_TEST === "1" ? "mock" : undefined,
});
```

`forceModel` is the one runtime-enforced switch: a provider pinned inside a dependency cannot leak a real call into a test run. Values returned by a mock are still validated against the extractor's schema, so a mock that returns the wrong shape fails the test — useful in itself.
...
test("configure in code with a mock provider", async () => {
  ...
  nolaRuntime.configure({ model: mockProvider([{ name: "Bob", age: 40, employer: "Initech", job: "dev" }]) });
