...
Projects created with `npm create nola` (and retrofits with `--add`) carry a `.vscode/extensions.json` that recommends the extension, so VS Code offers to install it when the folder opens — the editor setup is written by default; `--ide none` skips it. The interactive scaffold also offers to install the dependencies and open the folder in VS Code as its last step (answer *yes* to *Install dependencies and open VS Code?*) — it lands on the entry file, whose opening comment lists the next steps (F5, a breakpoint, the extension). The offer needs VS Code's `code` command on your PATH — on macOS run *Shell Command: Install 'code' command in PATH* from the Command Palette once; without it the last question is just *Install dependencies?* and the folder is not opened.
