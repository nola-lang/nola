...
Projects created with `npm create nola` when you keep **VS Code** ticked in the setup list (the interactive default; pass `--ide vscode` non-interactively or for a retrofit) carry a `.vscode/extensions.json` that recommends the extension, so VS Code offers to install it when the folder opens. The interactive scaffold also offers to install the dependencies and open the folder in VS Code as its last step (answer *yes* to *Install dependencies and open VS Code?*); it needs VS Code's `code` command on your PATH — on macOS run *Shell Command: Install 'code' command in PATH* from the Command Palette once.
