...
Projects created with `npm create nola` when you pick **VS Code** at the editor prompt (the interactive default; pass `--ide vscode` non-interactively or for a retrofit) carry a `.vscode/extensions.json` that recommends the extension, so VS Code offers to install it when the folder opens.
